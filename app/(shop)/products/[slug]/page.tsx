...
export default function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  const variantImages = (product.variants || [])
    ...
    .filter(Boolean);
  const imagesList = Array.from(new Set([product.frontImage, product.backImage, ...variantImages].filter(Boolean)));
    ...
    <div className="min-h-screen bg-[#0d0d0d] text-white py-10 md:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          ...
          <div className="lg:col-span-6 lg:sticky lg:top-[185px] self-start pt-2">
            <div className="flex gap-3.5 items-start">
              {/* Vertical Column of Thumbnails */}
              <div className="flex flex-col gap-2 w-14 sm:w-16 flex-shrink-0 max-h-[500px] overflow-y-auto no-scrollbar [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                {imagesList.map((img, idx) => (
                  <button
                    key={idx}
                    onClick={() => {
                      setActiveImage(img);
                      const matchingVariant = activeVariants.find((v: any) => v.imageUrl === img);
                      if (matchingVariant) {
                        if (matchingVariant.productType) setSelectedType(matchingVariant.productType);
                    ...
                    className={`relative aspect-square w-full rounded-xl overflow-hidden bg-gray-100 dark:bg-[#181818] border-2 transition cursor-pointer ${
                      activeImage === img ? 'border-[#ff7700] ring-2 ring-[#ff7700]/30' : 'border-gray-200 dark:border-[#262626] opacity-70 hover:opacity-100'
                    }`}
                  >
              ...
              <div 
                ...
                className="flex-1 relative aspect-square rounded-3xl overflow-hidden bg-gray-100 dark:bg-[#181818] border border-gray-200 dark:border-[#262626] shadow-2xl group/main cursor-zoom-in"
              >
                <Image
          ...
          {/* RIGHT: Product Info & Compact Order Controls */}
          <div className="lg:col-span-6 space-y-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="category-pill inline-block bg-red-950/60 text-[#ff7700] border border-red-800/40 text-[11px] font-extrabold px-3 py-0.5 rounded-full uppercase tracking-wider shadow-sm">
                  {product.categoryLabel || product.category}
                </span>
                ...
                <div className="flex items-center gap-1 text-xs">
                  <div className="flex text-amber-400">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} size={13} fill={i < Math.round(displayedRating) ? 'currentColor' : 'none'} />
                    ))}
                  </div>
                  <span className="font-bold text-gray-900 dark:text-white text-xs">{displayedRating.toFixed(1)}</span>
                  <a href="#reviews" className="text-gray-400 hover:text-[#ff7700] underline text-[11px]">
                    ({displayedReviewCount} reviews)
                  </a>
                </div>
              </div>

              <h1 className="text-xl sm:text-2xl font-extrabold font-heading text-gray-900 dark:text-white leading-tight mb-2">
                {product.title}
              </h1>

              {/* Price Display */}
              <div className="flex items-baseline gap-3">
                <span className="text-2xl sm:text-3xl font-extrabold text-[#c2410c] dark:text-[#ff7700]">
                ...
                {(() => {
                  ...
                  return discount > 0 ? (
                    <span 
                      className="sale-badge text-[11px] bg-[#a80000] text-white px-2 py-0.5 rounded-full font-black uppercase tracking-wider shadow-sm"
            ...
            {/* Streamlined Live Social Proof & Urgency Strip */}
            <div className={`p-2.5 sm:p-3 bg-gradient-to-r from-orange-50/80 via-red-50/40 to-transparent dark:from-[#181818] dark:to-[#121212] border rounded-xl flex flex-wrap items-center justify-between gap-2 text-xs ${isOutOfStock ? 'border-gray-200 dark:border-gray-700' : 'border-orange-200/80 dark:border-red-900/40'}`}>
              <div className="flex items-center gap-2 font-bold text-gray-800 dark:text-gray-200 text-[11px] sm:text-xs">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                </span>
                <span className="text-emerald-600 dark:text-emerald-400 font-bold">14 viewing right now</span>
                <span className="text-gray-300 dark:text-gray-600">•</span>
                <span className={`flex items-center gap-1 font-extrabold ${isOutOfStock ? 'text-gray-500' : 'text-red-600 dark:text-red-400'}`}>
                  <Flame className={`w-3.5 h-3.5 ${isOutOfStock ? 'text-gray-400' : 'animate-bounce text-red-500'}`} />
              ...
              {!isOutOfStock && (
                <div className="flex items-center gap-1 text-[11px] font-medium text-gray-600 dark:text-gray-400">
                  <Clock className="w-3 h-3 text-[#c2410c] dark:text-[#ff7700]" />
                  <span>Fast dispatch in 24h</span>
                </div>
              )}
            </div>

            {/* COMPACT VARIANT CONTROLS FORM */}
            <div className="space-y-4 bg-white dark:bg-[#141414] p-4 sm:p-5 rounded-2xl border border-gray-200 dark:border-[#222] shadow-sm">
              
              {/* 1. Select Style */}
              <div>
                <label className="block text-[11px] font-bold uppercase text-gray-500 dark:text-gray-400 mb-1.5">
                  1. Select Style: <span className="text-gray-900 dark:text-white font-extrabold">{selectedType || 'Unavailable'}</span>
                </label>
                <div className="grid grid-cols-3 gap-2">
                  {availableTypes.map((type) => (
                    <button
                      ...
                      className={`py-2 px-2.5 text-xs font-bold rounded-xl border transition cursor-pointer text-center truncate ${
                        selectedType === type
                          ? 'border-[#ff7700] bg-orange-50 dark:bg-[#ff7700]/10 text-[#c2410c] dark:text-[#ff7700] ring-1 ring-[#ff7700] shadow-xs'
                          : 'border-gray-200 dark:border-[#2a2a2a] bg-gray-50 dark:bg-[#1e1e1e] text-gray-800 dark:text-gray-300 hover:border-[#ff7700]'
                      }`}
                    >
                      {type}
                    </button>
              ...
              <div>
                <label className="block text-[11px] font-bold uppercase text-gray-500 dark:text-gray-400 mb-1.5">
                  2. Select Color: <span className="text-gray-900 dark:text-white font-extrabold">{selectedColor || 'Unavailable'}</span>
                </label>
                <div className="flex flex-wrap gap-2.5">
                  {availableColors.map((colorOption) => {
                    const colorName = colorOption.name;
                    const variantForThisColor = variantsForType.find((variant: any) => variant.color === colorName);
                    return (
                      <button
                        key={colorName}
                        type="button"
                        ...
                        className={`color-swatch-pill relative w-8 h-8 rounded-full border-2 flex items-center justify-center transition shadow-xs cursor-pointer overflow-hidden ${
                          selectedColor === colorName 
                            ? 'border-[#ff7700] ring-2 ring-[#ff7700]/30 scale-110' 
                            : 'border-gray-300 dark:border-gray-700 hover:scale-105'
                        }`}
                        style={{ backgroundColor: colorOption.hex || '#333333' }}
                        title={colorName}
                      >
                        {variantForThisColor?.imageUrl && !colorOption.hex ? (
                          /* eslint-disable-next-line @next/next/no-img-element */
                          <img src={variantForThisColor.imageUrl} alt={colorName} className="w-full h-full object-cover" />
                        ) : null}
                        {selectedColor === colorName && (
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-[11px] font-bold uppercase text-gray-500 dark:text-gray-400">
                    3. Select Size: <span className="text-gray-900 dark:text-white font-extrabold">{selectedSize || 'Out of stock'}</span>
                  </label>
                  <button
                    type="button"
                    onClick={openSizeGuide}
                    className="text-[11px] font-bold text-[#c2410c] dark:text-[#ff7700] hover:underline flex items-center gap-1 cursor-pointer"
                  >
                    Size Guide 📏
                  </button>
                </div>
                <div className="grid grid-cols-6 gap-2">
                  {availableSizes.length > 0 ? availableSizes.map((sz) => (
                    <button
                      ...
                      onClick={() => {
                        setSelectedSize(sz);
                        const matched = variantsForColor.find((variant: any) => variant.size === sz);
                        if (matched?.imageUrl) {
                      ...
                      className={`py-2 text-xs font-bold rounded-xl border transition cursor-pointer text-center ${
                        selectedSize === sz
                          ? 'border-[#a80000] bg-[#a80000] text-white shadow-xs'
                          : 'border-gray-200 dark:border-[#2a2a2a] bg-gray-50 dark:bg-[#1e1e1e] text-gray-800 dark:text-gray-300 hover:border-gray-400 dark:hover:border-gray-600'
                      }`}
                    >
                      {sz}
                    </button>
                  )) : (
                    <p className="col-span-6 rounded-xl border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/30 px-3 py-2 text-xs font-semibold text-red-700 dark:text-red-300">
              ...
              <div className="pt-3 border-t border-gray-200 dark:border-[#262626] space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold uppercase text-gray-500 dark:text-gray-400">Quantity:</span>
                  <div className="flex items-center bg-gray-50 dark:bg-[#1e1e1e] border border-gray-200 dark:border-[#2a2a2a] rounded-xl">
                    <button
                      type="button"
                      onClick={() => setQuantity(Math.max(1, quantity - 1))}
                      className="px-3 py-1 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white font-bold cursor-pointer"
                    >
                      -
                    </button>
                    <span className="px-3 py-1 text-xs font-extrabold text-gray-900 dark:text-white">{quantity}</span>
                    <button
                      type="button"
                      onClick={() => setQuantity(Math.min(currentStock, quantity + 1))}
                      disabled={isOutOfStock || quantity >= currentStock}
                      className="px-3 py-1 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white font-bold disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                    >
                      +
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  <button
                    ...
                    className="add-to-cart-cta-btn w-full py-3 bg-[#0f172a] hover:bg-[#1e293b] text-white font-bold text-xs uppercase tracking-wider rounded-xl border border-[#1e293b] transition flex items-center justify-center gap-2 cursor-pointer disabled:bg-gray-700 disabled:text-gray-400 disabled:cursor-not-allowed shadow-md"
                  ...
                  <button
                    ...
                    disabled={isOutOfStock}
                    className="w-full py-3 bg-[#a80000] hover:bg-[#850000] text-white font-bold text-xs uppercase tracking-wider rounded-xl transition flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-red-950/20 disabled:bg-gray-700 disabled:text-gray-400 disabled:cursor-not-allowed"
                    style={{ backgroundColor: '#a80000', color: '#ffffff' }}
                  >
                    <span>⚡ Buy Now</span>
                  </button>
            ...
            <div className="space-y-3 pt-2">
              <div className="bg-white dark:bg-[#141414] border border-gray-200 dark:border-[#222] rounded-2xl overflow-hidden shadow-sm transition-colors">
                <button
                  onClick={() => setOpenTab(openTab === 'desc' ? ('' as any) : 'desc')}
                  className="w-full p-4 text-left font-bold text-xs uppercase tracking-wider text-gray-900 dark:text-white flex items-center justify-between hover:text-[#ff7700] transition cursor-pointer"
                >
                  <span>Description & Fabric Specs</span>
                  {openTab === 'desc' ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                </button>
                {openTab === 'desc' && (
                  <div className="p-4 pt-0 text-xs text-gray-700 dark:text-gray-300 leading-relaxed border-t border-gray-100 dark:border-[#222]">
